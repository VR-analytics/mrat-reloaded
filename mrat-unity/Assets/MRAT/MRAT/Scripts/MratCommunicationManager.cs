...
namespace MRAT
{
    public class MratCommunicationManager : MonoBehaviour
    {
        ...
        public bool DisplayLocalLogging = false;
        public GameObject DisplayLocalLoggingObject;
        public bool LogtoFile = false;
        string logfilename = "VRAnalyticsLog";
	    ...
	    private void Awake()
	    {
            ...
            logfilename= logfilename + System.DateTime.Now.ToString("yyyy_MM_dd_HH_mm_ss") + ".txt";
		...
		public void LogMratEvent(MratEventSimple mratEvent)
        {
            ...
            if (LocalLoggingOnly)
            {
                if (LogGeneratedMessagesLocally)
                    Debug.Log($"MratEventSimple ({mratEvent.EventType}) logged only locally: {mratEvent.ToString()}");
                if (DisplayLocalLogging)
                    DisplayLocalLoggingObject.GetComponent<TextMeshPro>().text = $"MratEventSimple ({mratEvent.EventType}) logged only locally: {mratEvent.ToString()}";
                if (LogtoFile)
                {
                    if(LogHeaderInsertion)
                    {
                        WriteToLog($"Event Type\tEvent Information");
                        LogHeaderInsertion = false;
                    }
                    WriteToLog($"{mratEvent.EventType}\t{mratEvent.ToString()}");
                }
            }
            else
            {
